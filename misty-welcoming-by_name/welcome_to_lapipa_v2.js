...
function _FaceDetect(data) {
  ...
  if (faceDetected == "unknown person") {
    ...
    misty.Speak(
      "Hi " +
        faceDetected.normalize("NFD").replace(/[\u0300-\u036f]/g, "") +
        ", we've met before! How are you?",
    ); // Normalize & replace remove accents
...
function _GetAudioFile(data) {
  ...
  misty.Speak("Nice to meet you!");

  let base64 = data.Result.Base64;

  /* For the program to run in Misty:
        1. Comment the line below (181) defining the APIKEY, otherwise the code holds
        2. Replace apikey with your API key directly

        In short, using global variables holds the code and it doesn't work
    */
  let apikey = APIKEY;
...
function _SendExternalRequest(data) {
  // When we get here, we ALWAYS have a response
  ...
  if (!response.results[0].alternatives[0].transcript) {
    // This checks if the person's name was obtained
    ...
    misty.Debug(
      "Value of label is: " + response.results[0].alternatives[0].transcript,
    );
    misty.Speak("<speak> Please stand still for a second. </speak>");
